...
def deconvolve(file_absolute_path, experiment_label, properties):
    try:
        ...
        for i in range(n_lorentz):
            ...
            plt.fill(x, y,
                     ...
                     alpha=0.1)
            peak_df = pd.DataFrame(list(zip(x, y)), columns=["#Wave", "#Intensity"])
            peak_df.to_csv(path_or_buf=path_utils.join(output_dir, file_name_root + f".lorentz_peak{i + 1}" + ".txt"),
                           sep="\t", index=False)
            peaks["#"].insert(peak_index, f"%_{peak_index + 1}")
